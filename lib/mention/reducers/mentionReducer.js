...
var actionsMap = {
  ...
  query: function query(state, action) {
    ...
    var matchedSources = state.dataSource.filter(function (source) {
      if (query.length) {
        var noSpaceQuery = newQuery.replace(/\s/g, '');

        return source.searchKey.toLowerCase().replace(/\s/g, '').includes(noSpaceQuery);
      } else {
        return false;
      }
    });
  ...
  remove: function remove(state, action) {
    ...
    var foundMention = (0, _utilsLast2['default'])(mentions.filter(function (source) {
      ...
      return displayLabel && displayLabel.toLowerCase().includes(mention.toLowerCase());
    }));
  ...
  syncEditorState: function syncEditorState(state, action) {
    ...
    return filteredMentions.length ? { mentions: filteredMentions } : {};
  }
};

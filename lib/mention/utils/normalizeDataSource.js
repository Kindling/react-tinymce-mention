...
function validateDataSource(dataSource) {
  ...
  if ((0, _containsConsistantType2['default'])(dataSource, 'object')) {
    ...
    var isValid = dataSource.every(function (s) {
      return s.hasOwnProperty('displayLabel') && typeof s.displayLabel === 'string' && s.hasOwnProperty('searchKey') && typeof s.searchKey === 'string';
    });
    ...
    // Array containing simple strings, e.g., ['dimitri', 'shostakovich']
      ...
      throw new Error('Validation Error: `transformedDataSource` must be an array of strings ' + 'or contain objects with a `displayLabel` and `searchKey` property.');
    }
}
